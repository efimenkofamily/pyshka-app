...
function checkStoreStatus() {
    // Админам и разработчикам можно всегда
    if (currentUserData && (currentUserData.status === 'admin' || currentUserData.status === 'developer')) return true;
...
async function checkUser() {
    ...
    try {
        ...
        const data = await response.json();
            ...
            // Если статус admin или developer — даем доступ к админке!
            if (currentUserData.status === 'admin' || currentUserData.status === 'developer') {
                const badge = document.getElementById('profileStatusBadge');
                
                if (currentUserData.status === 'developer') {
                    badge.innerText = 'Разработчик';
                    badge.style.background = 'rgba(52, 152, 219, 0.15)'; // Синий фон
                    badge.style.color = '#3498db'; // Синий текст
                } else {
                    badge.innerText = 'Администратор';
                    badge.style.background = 'rgba(255, 59, 48, 0.15)'; // Красный фон
                    badge.style.color = '#FF3B30'; // Красный текст
                }
                
                // Показываем скрытую кнопку Админ-панели для обоих
...
function renderAdminUsers(users) {
    ...
    users.forEach(u => {
        ...
        const statusColors = {
            ...
            'developer': '#3498db' // Синий для разработчика
        };
        ...
        <div class="admin-order-card" style="border-left: 4px solid ${color}; position: relative;">
            ...
            <div style="margin-top: 15px; border-top: 1px dashed rgba(232, 195, 150, 0.3); padding-top: 15px;">
                ...
                <select class="address-input" style="margin:0; padding:10px; font-size:13px; font-weight:bold; color:${color}; border-color:${color}; outline:none;" onchange="changeUserStatus(${u.id}, this.value)">
                    ...
                    <option value="developer" ${u.status === 'developer' ? 'selected' : ''}>💻 Разработчик</option>
...
async function saveAdminSettings() {
    ...
    try {
        // 1. Собираем Настройки наценок
        let newCatMargins = {};
        for (let cat in STORE_SETTINGS.categoryMargins) {
            newCatMargins[cat] = parseFloat(document.getElementById(`set-margin-${cat}`).value);
        }

        // 2. Достаем даты из инпутов и переводим в формат ISO для базы данных
        const openDateVal = document.getElementById('set-open-date').value;
        const closeDateVal = document.getElementById('set-close-date').value;
        
        STORE_SETTINGS.openDate = openDateVal ? new Date(openDateVal).toISOString() : null;
        STORE_SETTINGS.closeDate = closeDateVal ? new Date(closeDateVal).toISOString() : null;
        ...
        // 3. Формируем объект для отправки (теперь с датами!)
        const newConfigDB = {
            ...
            open_date: STORE_SETTINGS.openDate,    // <-- ДОБАВИЛИ ДАТУ ОТКРЫТИЯ
            close_date: STORE_SETTINGS.closeDate   // <-- ДОБАВИЛИ ДАТУ ЗАКРЫТИЯ
        };

        // Сохраняем конфиг в базу
        ...
        // 4. Сохраняем Статусы заказов (отправляем каждый измененный статус в базу)
        ...
        // Сразу проверяем статус магазина после сохранения новых дат
        if (!checkStoreStatus()) {
            startDropTimer();
            const closedScreen = document.getElementById('closedScreen');
            if (closedScreen) closedScreen.style.display = 'flex';
        ...
        renderProducts(); // Перерисовываем каталог
    } catch (e) {
        console.error(e);
